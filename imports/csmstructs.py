...
class Memory:
    ...
    def __calculate_address(self, address: int) -> int:
        """Will convert an abstract address into an actual index to access a value in the memory pool array"""

        pointer: int = self.__number_registers + address

        if -1 < pointer < self.__memory_pool_length:
