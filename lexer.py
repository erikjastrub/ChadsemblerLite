...
class Lexer:
    ...
    def __get_token(self) -> str:
        ...
        while   self.__index < self.__length and \
                ...
                addressingmodes.get_addressing_mode(self.__source_code[self.__index]) is None:
    ...
    def __get_type(self, token: str) -> int:
        ...
        if instructions.get_instruction(token) is not None:

            return tokentypes.INSTRUCTION

        elif registers.get_spr(token) is not None:

            return tokentypes.REGISTER

        elif addressingmodes.get_addressing_mode(token) is not None:
    ...
    def __handle_value(self, token: str) -> None:
        ...
        index: int = 1 if token[0] in lexerdefaults.VALUE_SIGNS else 0
    ...
    def __tokenise(self) -> list[TypedToken]:
        ...
        while self.__index < self.__length:
            ...
            elif addressingmodes.get_addressing_mode(current_char) is not None:
